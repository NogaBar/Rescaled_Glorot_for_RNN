...
from model import LRU, RNN, DiagRNN
...
def train(args):
    ...
    if args.use_wandb:
        ...
        config = wandb.config  # Get W&B config (overrides args if sweep is running)
    else:
        wandb.init(mode="offline")
        config = vars(args)

    lr = args.lr_base
    # 3. Get the learning rate from either args or W&B
    ssm_lr = config.lr_factor * lr
    ...
    bidirectional = True if 'path' in args.dataset else False
    if args.model == 'lru':
        lru = partial(
            LRU, d_hidden=args.d_hidden, d_model=args.d_model, r_min=args.r_min, r_max=args.r_max,
            bidirectional=bidirectional, max_phase=args.max_phase, A_init=args.A_init
        )
        model_cls = partial(
        ...
        rnn = partial(
            RNN, d_hidden=args.d_hidden, d_model=args.d_model, r_min=args.r_min, r_max=args.r_max,
            A_init=args.A_init, bidirectional=bidirectional
        )
        model_cls = partial(
            BatchClassificationModel,
            lru=rnn,
    ...
    elif args.model == 'diag-rnn':
        diag_rnn = partial(
            DiagRNN, d_hidden=args.d_hidden, d_model=args.d_model, r_min=args.r_min, r_max=args.r_max,
            A_init=args.A_init, bidirectional=bidirectional
        )
        model_cls = partial(
            BatchClassificationModel,
    ...
    for epoch in range(args.epochs):
        ...
        if jnp.isnan(train_loss) or jnp.isinf(train_loss):
            print(f"[*] Epoch {epoch + 1} Train Loss is NaN or Inf. Exiting...")
            break
