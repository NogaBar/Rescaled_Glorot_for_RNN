...
DEFAULT_CACHE_DIR_ROOT = Path("lru/cache_dir/")
...
def create_lra_imdb_classification_dataset_new(cache_dir: Union[str, Path] = DEFAULT_CACHE_DIR_ROOT, batch_size: int = 50, seed: int = 42
) -> ReturnType:
    print("[*] Generating LRA-text (IMDB) Classification Dataset")
    name = "imdb"
    from dataloaders.lra_config import get_text_classification_config
    from dataloaders.lra_datasets import ImdbDataset
    config = get_text_classification_config()
    train_dataset = ImdbDataset(config, split='train')
    test_dataset = ImdbDataset(config, split='eval')

    trainloader = make_data_loader(
        train_dataset, None, seed=seed, batch_size=batch_size
    )
...
def create_lra_listops_classification_dataset(
    ...
    name = "listops"
    dir_name = "lru/raw_datasets/lra_release/lra_release/listops-1000"
...
def create_lra_listops_classification_dataset_new(
    cache_dir: Union[str, Path] = DEFAULT_CACHE_DIR_ROOT, batch_size: int = 50, seed: int = 42
) -> ReturnType:
    print("[*] Generating LRA-listops Classification Dataset")
    from dataloaders.lra_config import get_listops_config
    from dataloaders.lra_datasets import ListOpsDataset
    config = get_listops_config()
    train_dataset = ListOpsDataset(config, split='train')
    test_dataset = ListOpsDataset(config, split='eval')
...
def create_lra_path32_classification_dataset(
    ...
    name = "pathfinder"
    resolution = 32
    dir_name = f"lru/raw_datasets/lra_release/lra_release/pathfinder{resolution}"
...
def create_lra_aan_classification_dataset(
    ...
    name = "aan"
    dir_name = "lru/raw_datasets/lra_release/lra_release/tsv_data"
    kwargs = {
...
Datasets = {
    ...
    # LRA.
    "imdb-classification": create_lra_imdb_classification_dataset, # text
    "listops-classification": create_lra_listops_classification_dataset, # retrival
