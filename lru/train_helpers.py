...
def create_train_state(model_cls, rng, in_dim, batch_size, seq_len, weight_decay, norm, ssm_lr, lr):
    ...
    ssm_fn = map_nested_fn(
        lambda k, _: "ssm"
        if k in ["nu_log", "theta_log", "gamma_log", "B_re", "B_im", "B", "A"]
        else "regular"
    )
...
def prep_batch(batch, seq_len, in_dim):
    ...
    # Inputs size is [n_batch, in_dim, seq_len] make it [n_batch, seq_len, in_dim].
    if len(inputs.shape) >2 and (inputs.shape[1] < inputs.shape[2]):
