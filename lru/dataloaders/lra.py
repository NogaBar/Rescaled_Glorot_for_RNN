...
class IMDB(SequenceDataset):
    ...
    def process_dataset(self):
        ...
        if self.level == "word":
            tokenizer = torchtext.data.utils.get_tokenizer(None, language="en_core_web_sm")
        ...
        vocab = torchtext.vocab.build_vocab_from_iterator(dataset["train"]["tokens"])


        def numericalize(example):
            return {
                "input_ids": [vocab[t] for t in example["tokens"]]
            }
...
class ListOps(SequenceDataset):
    ...
    def process_dataset(self):
        ...
        def numericalize(example):
            return {
                "input_ids": [vocab[t] for t in example["tokens"]] # migth need to change
            }
